...
fisopfs_save_fs_on_exit()
{
	...
	if (!archivo_disco) {
		fprintf(stderr, "[Debug] ERROR al guardar fisop: %s\n", strerror(errno));
		return;
	}
...
fisopfs_getdir(const char *path, fuse_dirh_t h, fuse_dirfil_t filler)
{
	...
	for (int i = 0; i < MAX_INODES; i++) {
		if (super_b.bitmap_inodes[i] &&
			strcmp(super_b.inodes[i].directory_path, inodo_dir->path) == 0) {
...
main(int argc, char *argv[])
{
    ...
    if (temp_disk_path[0] != '/') {
        ...
        if (getcwd(current_dir, sizeof(current_dir)) != NULL) {
            if (strlen(current_dir) + 1 + strlen(temp_disk_path) + 1 > MAX_PATH) {
    ...
    printf("init - cargando el estado del sistema de archivos desde la ruta absoluta: %s\n", nombre_archivo_disco);
